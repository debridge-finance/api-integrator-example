...
async function main() {
  ...
  const usdmDecimals = 18; // USDM on MegaETH has 18 decimals
  const amountToSend = "0.2"; // The amount of USDM to send

  const amountInAtomicUnit = ethers.parseUnits(amountToSend, usdmDecimals);

  const sameChainSwapInput: SameChainSwapInput = {
    chainId: CHAIN_IDS.MegaETH.toString(),
    tokenIn: USDM.MegaETH,
  ...
  console.log(` Token to approve: ${sameChainSwapInput.tokenIn} (MegaETH USDM)`);
  console.log(` Spender address: ${spenderAddress}`);
  console.log(` Amount required: ${formatUnits(amountInAtomicUnit, usdmDecimals)} USDM`);
  ...
  try {
    ...
    console.log(` Current allowance: ${formatUnits(currentAllowance, usdmDecimals)} USDM`);
    ...
    if (currentAllowance < requiredAmountBigInt) {
      ...
      console.log(` --> View on MegaETH Blockscout: https://megaeth.blockscout.com/tx/${approveTxResponse.hash}`);
